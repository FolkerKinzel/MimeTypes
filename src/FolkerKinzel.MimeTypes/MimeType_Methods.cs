...
public sealed partial class MimeType
{
    ...
    /// <summary>
    /// Creates a new <see cref="MimeType"/> object that's filled with the data of an existing 
    /// <see cref="MimeTypeInfo"/> instance.
    /// </summary>
    /// <param name="info">The <see cref="MimeTypeInfo"/> instance whose data will be copied into the 
    /// <see cref="MimeType"/>.</param>
    /// <returns>A reference to the <see cref="MimeType"/> that is created.</returns>
    /// <exception cref="ArgumentException"><paramref name="info"/> is empty. (See 
    /// <see cref="MimeTypeInfo.IsEmpty">MimeTypeInfo.IsEmpty</see>.)</exception>
    ...
    /// <param name="language">An IETF-Language tag that indicates the language of the parameter's 
    /// value.</param>
    /// <returns>A reference to the <see cref="MimeType"/> instance on which the method was called.
    /// </returns>
    /// 
    /// <remarks>
    /// The <paramref name="key"/> of a parameter must be unique inside of a <see cref="MimeTypeInfo"/>.
    /// It's compared case insensitive. If this method is called several times with equal keys, the last 
    /// wins.
    /// </remarks>
    /// 
    ...
    /// </para>
    /// <para>
    /// <paramref name="language"/> is neither <c>null</c> nor <see cref="string.Empty"/> nor a valid 
    /// IETF-Language-Tag according to RFC-1766.
    /// </para>
    /// </exception>
    /// 
    /// <example>
    ...
    /// </summary>
    /// <param name="key">The <see cref="MimeTypeParameter.Key"/> of the <see cref="MimeTypeParameter"/> 
    /// to remove.</param>
    ///  <returns>A reference to the <see cref="MimeType"/> instance on which the method was called.</returns>
    ///  
    /// <example>
    ...
    /// </summary>
    /// <param name="value">The <see cref="ReadOnlyMemory{T}">ReadOnlyMemory&lt;Char&gt;</see> to parse.
    /// </param>
    /// <returns>The <see cref="MimeType"/> instance that <paramref name="value"/> represents.</returns>
    /// 
    ...
    /// <see cref="MimeType"/> parsed from <paramref name="value"/>. The parameter is passed uninitialized.
    /// </param>
    /// <returns><c>true</c> if <paramref name="value"/> could be parsed as <see cref="MimeType"/>; 
    ...
    /// <returns><c>true</c> if <paramref name="value"/> could be parsed as <see cref="MimeType"/>; otherwise,
    /// <c>false</c>.</returns>
    ...
    /// </summary>
    /// <param name="includePeriod"><c>true</c> specifies, that the period "." (U+002E) is included in 
    /// the retrieved file type extension, <c>false</c>, that it's not.</param>
    ...
    /// <see cref="MimeCache.EnlargeCapacity(int)">MimeCache.EnlargeCapacity(int)</see> or you can
    /// delete it with <see cref="MimeCache.Clear()">MimeCache.Clear()</see> if your application does not 
    /// need it anymore.
    /// </para>
    /// </remarks>
    ...
    /// file name.
    /// </summary>
    /// <param name="fileName">A file path, file name, file type extension (no matter whether with or without
    /// the period "."), or <c>null</c>.</param>
