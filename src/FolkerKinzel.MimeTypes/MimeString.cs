...
/// <summary>
/// Static class that works on <see cref="string"/>s and offers methods that perform conversions
...
public static class MimeString
{
    ...
    /// </summary>
    /// <param name="mimeType">A <see cref="string"/> that represents an Internet Media Type ("MIME type")
    /// or <c>null</c>.</param>
    /// <param name="includePeriod"><c>true</c> specifies, that the period "." (U+002E) is included in the 
    /// retrieved file type extension, <c>false</c>, that it's not.</param>
    ...
    /// </para>
    /// <para>
    /// This method doesn't perform any validation on <paramref name="mimeType"/>. If a strong validation 
    /// of the input is needed, the instance methods 
    /// <see cref="MimeTypeInfo.GetFileTypeExtension(bool)">MimeTypeInfo.GetFileTypeExtension(bool)</see> and
    /// <see cref="MimeType.GetFileTypeExtension(bool)">MimeType.GetFileTypeExtension(bool)</see> are better
    /// suited.
    /// </para>
    /// <para>
    ...
    /// <see cref="MimeCache.EnlargeCapacity(int)">MimeCache.EnlargeCapacity(int)</see> or you can delete it with
    /// <see cref="MimeCache.Clear()">MimeCache.Clear()</see> if your application does not need it anymore.
    /// </para>
    /// </remarks>
    /// 
    ...
    /// <param name="includePeriod"><c>true</c> specifies, that the period "." (U+002E) is included in the retrieved
    /// file type extension, <c>false</c>, that it's not.</param>
    /// <returns>An appropriate file type extension for <paramref name="mimeType"/>.</returns>
    /// <remarks>
    /// <para>
    /// If no other file type extension can be found, <see cref="MimeCache.DefaultFileTypeExtension"/>
    ...
    /// </para>
    /// <para>
    /// This method doesn't perform any validation on <paramref name="mimeType"/>. If a strong validation of the input
    /// is needed, the instance methods 
    /// <see cref="MimeTypeInfo.GetFileTypeExtension(bool)">MimeTypeInfo.GetFileTypeExtension(bool)</see> and
    /// <see cref="MimeType.GetFileTypeExtension(bool)">MimeType.GetFileTypeExtension(bool)</see> are better suited.
    /// </para>
    /// <para>
    /// Internally a small memory cache is used to retrieve often used file type extensions faster. You
    /// can enlarge the size of this cache with <see cref="MimeCache.EnlargeCapacity(int)">MimeCache.EnlargeCapacity(int)</see>
    /// or you can delete it with <see cref="MimeCache.Clear()">MimeCache.Clear()</see> if your application does not 
    /// need it anymore.
    /// </para>
    /// </remarks>
    ///
